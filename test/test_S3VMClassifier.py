...
class Test(unittest.TestCase):
    ...
    def generate_simple_dataset(self):
        ...
        """
        neg=self.generate_from_point([-1,-1],20,1.5,-1)
        pos=self.generate_from_point([3,3],20,1.5,1)
        labeled=pos+neg
        #labeled=[LabeledExample([-0.9,-0.9],-1),LabeledExample([0.9,-0.9],1),LabeledExample([-0.9,0.9],1),LabeledExample([0.9,0.9],1)]
        unlabeled=self.generate_from_point([0.570710,0.570710],50,1,0)
        #unlabeled=self.generate_from_function(lambda x:0.6*x+1,50,0.2,-3,3)
